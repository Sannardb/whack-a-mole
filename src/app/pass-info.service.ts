...
export class PassInfoService {
  ...
  countdownTimer() {
    ...
    this.state.countdownTimer = 15; // just to be able to play the game again without a reset button
    this.state.points = 0;
    this.state.gameOver = '';
    this.state.moles = 0;

    this.interval = setInterval(() =>
      // Set interval every 1s to execute timer
      {
        this.state.countdownTimer--;

        if (this.state.countdownTimer <= 0) {
          // if timer is less than or 0, clear interval and reset below
    ...
    setInterval(() => {
      if (this.state.activeGame == true) {
        ...
        if (this.state.chanceNumber > 20 && this.state.moles < 3) {
          // if the chanceNumber is bigger than 20 and moles are less than 3, run code below
          ...
          if (this.state.squares[this.randomHole] == false) {
            // if the square position is not busy, show a mole
          ...
          this.clearTimeout = setTimeout(
            (
              hole: number // run this timeout after 4 seconds
            ) => {
              if (this.state.squares[hole] == true) {
                // to not get more moles than we want we need this if statement
  ...
  whackedMole(i: number) {
    // get the index as parameter from the template
    console.log('whacked a mole');
    if (this.state.squares[i] == true) {
      // if the index from the template is true, set it to false to make it disappear in the template, add one point and remove a mole
